...
class DenseFeatureReader(object):
    ...
    def read_data(self, table_name, namespace, mode="fit"):
        ...
        for i, data in enumerate(data_instance.collect()):
            LOGGER.info("id:{}, date_instanceid:{}, data_instancefeatures:{}, shape:{}".format(
                                                                                        data[0], 
                                                                                        data[1].inst_id, 
                                                                                        data[1].features,
                                                                                        type(data[1].features)
                                                                                        )
            )
            if i == 3:
                break
        LOGGER.info("data_type:{}".format(self.data_type))
