    ...
    def check_param(workflow_param):
        ...
        workflow_param.method = check_and_change_lower(workflow_param.method,
                                                       ['train', 'predict', 'cross_validation',
                                                        ...
                                                        "one_vs_rest_predict", "neighbors_sampling", "network_embedding"],
                                                       descr)
...
class NeighborsSamplingChecker(object):
    @staticmethod
    def check_param(neighbors_sampling_param):
        desr = "neighbors_sampling_param's"

        check_positive_number(neighbors_sampling_param.times_of_sampling, desr + "times_of_sampling")
        check_positive_number(neighbors_sampling_param.w, desr + "w")
    
        LOGGER.debug("Finish neighbors sampling parameter check!")
        return True

class NetworkEmbeddingChecker(object):
    @staticmethod
    def check_param(network_embedding_param):
        descr = "network_embedding_param's"

        if type(network_embedding_param.dim).__name__ != 'int':
            raise ValueError(
                "network_embedding_param's dim {} not supported, should be int type".format(network_embedding_param.dim))
        
        if type(network_embedding_param.n_node).__name__ != 'int':
            raise ValueError(
                "network_embedding_param's n_node {} not supported, should be int type".format(network_embedding_param.n_node))

        if type(network_embedding_param.eps).__name__ != "float":
            raise ValueError(
                "network_embedding_param's eps {} not supported, should be float type".format(network_embedding_param.eps))

        if type(network_embedding_param.optimizer).__name__ != "str":
            raise ValueError(
                "network_embedding_param's optimizer {} not supported, should be str type".format(network_embedding_param.optimizer))
        else:
            network_embedding_param.optimizer = network_embedding_param.optimizer.lower()
            if network_embedding_param.optimizer not in ['sgd', 'rmsprop', 'adam', 'adagrad']:
                raise ValueError(
                    "network_embedding_param's optimizer not supported, optimizer should be"
                    " 'sgd', 'rmsprop', 'adam' or 'adagrad'")

        if type(network_embedding_param.batch_size).__name__ != "int":
            raise ValueError(
                "network_embedding_param's batch_size {} not supported, should be int type".format(network_embedding_param.batch_size))
        if network_embedding_param.batch_size != -1:
            if type(network_embedding_param.batch_size).__name__ not in ["int", "long"] \
                    or network_embedding_param.batch_size < consts.MIN_BATCH_SIZE:
                raise ValueError(descr + " {} not supported, should be larger than 10 or "
                                            "-1 represent for all data".format(network_embedding_param.batch_size))

        if type(network_embedding_param.learning_rate).__name__ != "float":
            raise ValueError(
                "network_embedding_param's learning_rate {} not supported, should be float type".format(
                    network_embedding_param.learning_rate))

        InitParamChecker.check_param(network_embedding_param.init_param)

        if type(network_embedding_param.max_iter).__name__ != "int":
            raise ValueError(
                "network_embedding_param's max_iter {} not supported, should be int type".format(network_embedding_param.max_iter))
        elif network_embedding_param.max_iter <= 0:
            raise ValueError(
                "network_embedding_param's max_iter must be greater or equal to 1")

        if type(network_embedding_param.converge_func).__name__ != "str":
            raise ValueError(
                "network_embedding_param's converge_func {} not supported, should be str type".format(
                    network_embedding_param.converge_func))
        else:
            network_embedding_param.converge_func = network_embedding_param.converge_func.lower()
            if network_embedding_param.converge_func not in ['diff', 'abs']:
                raise ValueError(
                    "network_embedding_param's converge_func not supported, converge_func should be"
                    " 'diff' or 'abs'")

        EncryptParamChecker.check_param(network_embedding_param.encrypt_param)

        if type(network_embedding_param.re_encrypt_batches).__name__ != "int":
            raise ValueError(
                "network_embedding_param's re_encrypt_batches {} not supported, should be int type".format(
                    network_embedding_param.re_encrypt_batches))
        elif network_embedding_param.re_encrypt_batches < 0:
            raise ValueError(
                "network_embedding_param's re_encrypt_batches must be greater or equal to 0")

        if type(network_embedding_param.model_path).__name__ != "str":
            raise ValueError(
                "network_embedding_param's model_path {} not supported, should be str type".format(
                    network_embedding_param.model_path))

        if type(network_embedding_param.table_name).__name__ != "str":
            raise ValueError(
                "network_embedding_param's table_name {} not supported, should be str type".format(
                    network_embedding_param.table_name))

        if type(network_embedding_param.party_weight).__name__ not in ["int", 'float']:
            raise ValueError(
                "network_embedding_param's party_weight {} not supported, should be 'int' or 'float'".format(
                    network_embedding_param.party_weight))

        LOGGER.debug("Finish network embedding parameter check!")
        return True
...
class AllChecker(object):
    ...
    def check_all(self):
        ...
        self._check(param.NeighborsSamplingParam, NeighborsSamplingChecker)
        self._check(param.NetworkEmbeddingParam, NetworkEmbeddingChecker)
