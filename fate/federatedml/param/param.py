...
class WorkFlowParam(object):
    ...
    def __init__(self, method='train', train_input_table=None, train_input_namespace=None, model_table=None,
                 ...
                 one_vs_rest=False, need_one_hot=False, local_samples_namespace=None, distributed_samples_namespace=None):
        self.method = method
        ...
        self.local_samples_namespace = local_samples_namespace
        self.distributed_samples_namespace = distributed_samples_namespace
...
class NeighborsSamplingParam(object):
    """
    Define the neighbors sampling parameters.

    Parameters
    ----------
    times_of_sampling: int. How many times will we do sampling for each node.
    w: int. The size of context windows. 
    """
    def __init__(self, times_of_sampling=80, w=5, nega_samp_num=5):
        self.times_of_sampling = times_of_sampling
        self.w = w
        self.nega_samp_num = nega_samp_num

class NetworkEmbeddingParam(object):
    """
    Define the network embedding parameters
    
    Parameters
    ----------
    dim:  int, default: 128
        The dimension of of embedding
    ...
    def __init__(self,
                 dim=128, n_node=0, eps=1e-5, optimizer='sgd', party_weight=1,
                 batch_size=-1, learning_rate=0.01, init_param=InitParam(fit_intercept=False),
                 max_iter=20, converge_func='diff',
                 encrypt_param=EncryptParam(), re_encrypt_batches=2,
                 model_path='ne_model', table_name='ne_table',
                 encrypted_mode_calculator_param=EncryptedModeCalculatorParam()):
        self.dim = dim
        self.n_node = n_node
        self.eps = eps
