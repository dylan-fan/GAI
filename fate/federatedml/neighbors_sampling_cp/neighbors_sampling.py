...
class NeighborsSampling(object):
    ...
    def local_neighbors_sampling(self, data_instances, role):
        ...
        # name = role
        # namespace = "neighbors_sampling/local_neighbors_sampling"
        # local_instances.save_as(name, namespace, partition=10)
        # LOGGER.info("Local neighbors pairs saved namespace:{}, name:{}".format(namespace, name))
    ...
    def _distributed_sampling_src(self, bridge_instances, src=consts.HOST, dst=consts.GUEST):
        ...
        LOGGER.info("Generate the distributed {} samples for {}".format(src, src))
        number_instances = federation.get(name=number_transfer.name,
                                 tag=self.transfer_variable.generate_transferid(number_transfer),
                                 idx=0)
        LOGGER.info("Get numbers from {}".format(dst))
        #logDtableInstances(LOGGER, number_instances, topk=5, isInstance=False)

        sample_id_instances, distributed_instances = NeighborsSampling.gen_distributed_sample_ids(bridge_instances, number_instances, src)
        LOGGER.info("distributed_instances count:{}".format(distributed_instances.count()))
        #logDtableInstances(LOGGER, sample_id_instances, topk=5)
        #logDtableInstances(LOGGER, distributed_instances, topk=5, isInstance=False)

        name = src
        namespace = 'neighbors_sampling/distributed_neighbors_sampling/distributed_{}_sampling'.format(src)
        distributed_instances.save_as(name, namespace, partition=10)
        LOGGER.info("store the distributed_{} samples for {} in namespace:{}, name:{}".format(src, src, namespace, name))
        ...
        federation.remote(sample_id_instances,
                          ...
                          role=dst,
                          idx=0)
        LOGGER.info("Remote generated distributed samples' id to {}".format(dst))
        
        # distributed_instances_src (the input node in skip-gram which is the part of sample)

        return distributed_instances
    

    def _distributed_sampling_dst(self, bridge_instances, src=consts.HOST, dst=consts.GUEST):
        ...
        LOGGER.info("Generate the distributed {} samples for {}".format(src, dst))
        ...
        federation.remote(number_instances,
                          ...
                          role=src,
                          idx=0)
        LOGGER.info("Remote numbers to {}".format(src))

        sample_id_instances = federation.get(name=sample_id_transfer.name,
                                             tag=self.transfer_variable.generate_transferid(sample_id_transfer),
                                             idx=0)
        LOGGER.info("Get generated distributed samples' id from {}".format(src))
        ...
        LOGGER.info("The number of {} distributed_instances:{}".format(src, distributed_instances.count()))
        
        name = dst
        namespace = 'neighbors_sampling/distributed_neighbors_sampling/distributed_{}_sampling'.format(src)
        distributed_instances.save_as(name, namespace, partition=10)
        LOGGER.info("store the distributed_{} samples for {} in namespace:{}, name:{}".format(src, dst, namespace, name))

        # distributed_instances_dst (to be predicted node in skip-gram which is the part of sample)
        return distributed_instances

    
    def _distributed_negative_sampling_src(self, positive_instances, src=consts.HOST, dst=consts.GUEST):
        ...
        distributed_negative_instances_src = positive_instances.map(lambda k, v: gen_neg_ids(k, v, self.nega_samp_num)).flatMap(lambda k, v: v)
        distributed_negative_ids = distributed_negative_instances_src.take(distributed_negative_instances_src.count(), keysOnly=True)
        ...
        federation.remote(distributed_negative_ids,
                          ...
                          role=dst,
                          idx=0)
        LOGGER.info("Remote the distributed negative instances id to {} from {}".format(dst, src))
        LOGGER.info("Distributed negative instances count: {}".format(len(distributed_negative_ids)))
        logDtableInstances(LOGGER, distributed_negative_instances_src, topk=10, isInstance=False)

        return distributed_negative_instances_src
