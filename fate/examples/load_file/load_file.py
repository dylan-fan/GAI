...
# ','.join([str(s) for s in input_list])
    ...
    if CSV in split_file_name:
        ...
        with open(input_file) as csv_file:
            ...
            for row in csv_reader:
                yield (row[0], list_to_str(row[1:]))        #row[0]为样本id
    else:
...
"""
transform raw data to distributed data format, 分布式数据抽象
"""
...
if __name__ == "__main__":
    ...
    try: 
        ...
        try:
            ...
            eggroll.init(mode=work_mode)     # work_mode has two values: standalone and cluster
