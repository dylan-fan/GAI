...
class WorkFlow(object):
    ...
    def train(self, train_data, validation_data=None):
        ...
        # The code above is to do feature engineering, and the next line is to train the model.
    ...
    def network_embedding(self, data_instances, node2id, local_instances, common_nodes):
        if self.mode == consts.HETERO and self.role != consts.ARBITER:
            LOGGER.debug("Enter {} network embedding function".format(self.role))
            LOGGER.debug("Start intersection before network embedding")
            intersect_flowid = "embedding0"
            data_instances = self.intersect(data_instances, intersect_flowid)
        LOGGER.info("{}".format(self.role))
        LOGGER.info("max iter: {}".format(self.nrler.max_iter))
        self.nrler.fit(data_instances, node2id, local_instances, common_nodes)
    ...
    def save_neighbors_sampling_result(self, neighbors_sampling_result):
        if neighbors_sampling_result:
            pass
        else:
            LOGGER.info("Not neighbros_sampling_result, do not save it!")
    ...
    def run(self):
        ...
        if self.workflow_param.method == "train":
        ...
        elif self.workflow_param.method == 'network_embedding':
            # assume neighbors sampling has done
            data_instances = None
            node2id = None
            ...
            if self.role != consts.ARBITER:
                if self.role == consts.HOST:
                    ...
                    name = 'target'
                data_instances = eggroll.table(name=name, namespace='neighbors_samples/distributed_samples/' + self.role, persistent=True)
                node2id = eggroll.table(name='node2id', namespace='node_id_map/' + self.role +'/')
                LOGGER.info("{} samples in {}".format(data_instances.count(), self.role))
                node2id = list(node2id.collect())
                node2id = dict(node2id)

                local_instances = eggroll.table(name=self.role, namespace='neighbors_samples/local_samples', persistent=True)
                common_nodes = eggroll.table(name='common_nodes', namespace='common_nodes', persistent=True)
                common_nodes = common_nodes.take(common_nodes.count(), keysOnly=True)
                common_nodes = [int(node) for node in common_nodes]

            self.network_embedding(data_instances, node2id, local_instances, common_nodes)
            LOGGER.debug("{}".format(self.role))
            LOGGER.info("Network_embedding_finish")
            ...
            if self.role == consts.HOST:
                ...
                o_name = 'Bank B'
            LOGGER.info("Obtain adjlist for each node for {}!".format(o_name))
            
            LOGGER.info("There are {} nodes in {}".format(adj_instances.count(), o_name))
            
            # save in DTable
            LOGGER.info("Map node to id:")
            nodes = adj_instances.take(adj_instances.count(), keysOnly=True)
            name = 'node2id'
            namespace = 'node_id_map/' + self.role +'/' 
            node2id = eggroll.table(name, namespace)
            #id2node = {}
            for id, node in enumerate(nodes):
                node2id.put(int(node), id)
                # id2node[id] = node
            #logDtableInstances(LOGGER, node2id, 5, isInstance=False)

            LOGGER.info("Enter the procedure of local neighbors sampling:")
            ...
            local_instances.save_as(name=self.role,
                                    ...
                                    partition=10)
            LOGGER.info("Local sampling samples count for {} is {}".format(self.role, local_instances.count()))
            LOGGER.info("Local sampling results for {} saved in namespace: {}, name: {}".format(self.role, self.workflow_param.local_samples_namespace, self.role))
            

            LOGGER.info("Enter the procedure of distributed neighbors sampling:")

            LOGGER.info("Alignment nodes from two parties:")
            intersect_flowid = 'neigh_sam_intersect_0'
            common_instance = self.intersect(data_instance, intersect_flowid)

            if self.role == consts.HOST:
                common_instance.save_as(name='common_nodes', namespace='common_nodes', partition=10)
            ...
            LOGGER.info("The number of bridge nodes: {}".format(bridge_instances.count()))
            ...
            distributed_instances_target.save_as(name="target",
                                              ...
                                              partition=10) 
            if self.role == consts.HOST:
                sample_primary = consts.GUEST
            else:
                sample_primary = consts.HOST
            LOGGER.info("Distributed sampling target results for {} saved in namespace: {}, name: {}".format(sample_primary, 
                                                                                                             self.workflow_param.distributed_samples_namespace + '/' + self.role, 
                                                                                                             'target'))
            ...
            distributed_instances_anchor.save_as(name='anchor',
                                              ...
                                              + self.role,
                                              partition=10)
            LOGGER.info("Distributed sampling anchor results for {} saved in namespace: {}, name: {}".format(self.role, 
                                                                                                             self.workflow_param.distributed_samples_namespace + '/' + self.role, 
                                                                                                             'anchor'))
